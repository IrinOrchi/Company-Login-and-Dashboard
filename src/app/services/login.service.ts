...
import { UserLoginInformation } from '../models/UserLoginInformationModel';
import {
  BehaviorSubject,
  Observable,
  catchError,
...
export class LoginService {
  ...
  private serviceInfoData = new BehaviorSubject<ServiceInfo | undefined>(undefined);
  ...
  getServiceData(companyId: string, userId: string): Observable<ServiceInfo> {
    return this.httpClient.post<ServiceInfo>(this.GET_SERVICE_DATA_ENDPOINT, {companyId, userId});
  }
    ...
    return this.getUserLoginData().pipe(
      map((data) => {
        if (data.SupportingInfo) {
          console.log("supporting info",data.SupportingInfo)
  ...
  setLegacyApiDataToLocalStorage(data: SupportingInfo) {
    this.gatewayDataSharingService.setSupportingInfoToSharedService(data);
    if (data) {
      console.log("setLegacyApiDataToLocalStorage info", data);
      
      // Set company size popup flag
      if (data.EmployeeMaxSize && data.EmployeeMaxSize > 0) {
        window.localStorage.setItem('companySizePopUp', 'submitted');
      } else {
        window.localStorage.setItem('companySizePopUp', '');
      ...
      window.localStorage.setItem(
        this.LOCAL_STORAGE_KEYS.Employee_Id,
        data.EmployeeId ? data.EmployeeId.toString() : '0'
      );
      window.localStorage.setItem(
        this.LOCAL_STORAGE_KEYS.USER_NAME,
        data.UserName
      ...
      window.localStorage.setItem(
        this.LOCAL_STORAGE_KEYS.Employee_Max_Size,
        data.EmployeeMaxSize.toString()
      );

      // Handle CV Search Service data
      if (data.CvSearchService) {
        window.localStorage.setItem(
          this.LOCAL_STORAGE_KEYS.CV_BANK_REMAINING,
          data.CvSearchService.Available
      ...
      // Get and set company ID and user ID
      this.getCompanyId().subscribe({
        next: (cId) => {
          if (cId) {
            console.log("company id cid", cId);
            window.localStorage.setItem(
              this.LOCAL_STORAGE_KEYS.COMPANY_ID,
              cId
            );
